    ...
    <AdminLayout>
      <div class="py-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <PieChart v-if="youthprofiles?.length" :data="youthprofiles" label="sex" :colors="['#FF6384', '#36A2EB']" />
          <BarChart v-if="youthprofiles?.length" :data="youthprofiles" label="baranggay" />
          <BarChart v-if="youthprofiles?.length" :data="youthprofiles" label="purok" />
          <DoughnutChart v-if="youthprofiles?.length" :data="civilStatusData" label="civil_status" :colors="['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0']" />
          <DoughnutChart v-if="youthprofiles?.length" :data="youthAgeGroupData" label="youth_age_group" :colors="['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0']" />
          <DoughnutChart v-if="youthprofiles?.length" :data="educationalBackground" label="educational_background" :colors="['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0']" />
          <DoughnutChart v-if="youthprofiles?.length" :data="youthClassification" label="youth_classification" :colors="['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0']" />
          <DoughnutChart v-if="youthprofiles?.length" :data="specificNeedsDetail" label="specific_needs_detail" :colors="['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0']" />
          <DoughnutChart v-if="youthprofiles?.length" :data="workStatus" label="work_status" :colors="['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0']" />
        </div>
      </div>
    </AdminLayout>
  </template>
